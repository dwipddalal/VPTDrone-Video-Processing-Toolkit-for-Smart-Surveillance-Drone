...
def inference(model, data_path, keys, eval_method):
    ...
    print(np.array(summary))
    np.save("/home/dwip.dalal/video_summarization/PGL-SUM/inference/naya_summary.npy", np.array(summary))
...
if __name__ == "__main__":
    ...
    for split_id in range(5):
        # Model data
        model_path = f"/home/dwip.dalal/video_summarization/PGL-SUM/inference/pretrained_models/table{table}_models/{dataset}/split{split_id}"
        model_file = [f for f in listdir(model_path) if isfile(join(model_path, f))]

        # Read current split
        # split_file = f"/home/dwip.dalal/video_summarization/PGL-SUM/data/datasets/splits/{dataset.lower()}_splits.json"
        ...
        # Dataset path
        dataset_path = f"/home/dwip.dalal/video_summarization/PGL-SUM/data/datasets/custom/naya_uff3.h5"
